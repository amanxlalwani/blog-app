...
app.post("/like/:id", async (c) => {
  ...
  if (!like) {
    const makeLike = await prisma.like.create({
      data: { postId: blogId, userId: userId, has_liked: true },
    });
  } else {
    const update = await prisma.like.update({
      where: { id: like.id, postId: blogId, userId: userId },
      data: { has_liked: !like.has_liked },
    });
  }
  if (like?.has_liked == true) return c.json({ message: "unliked the blog" });
...
app.post("/comment/:id", async (c) => {
  ...
  try {
    if (!body.isChildComment) {
      res = await prisma.comment.create({
        data: {
