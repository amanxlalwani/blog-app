...
<div className="flex justify-between px-4  mt-20 pb-8 lg:mt-4">
  ...
  <div className=" w-20 bg-green-600 flex justify-center items-center rounded cursor-pointer" onClick={async ()=>{
   ...
   if(blog.title=="" || blog.content==""){
    ...
    setLoading(true);
    try{
    const res=await axios.put("https://backend.aman-lalwani208.workers.dev/api/v1/blog/"+id,blog,{
        headers:{
            Authorization:localStorage.getItem('token')
        }
    })
...
<div className="flex flex-col w-10/12 lg:w-1/2 lg:gap-12 m-auto lg:mt-4 "> 
   <div>
   <input type="text" value={blog.title} className="lg:text-7xl text-3xl mt-8 lg:mt-0 w-full focus:outline-none" onChange={(e)=>{setBlog({...blog,title:e.target.value})}} placeholder="Title" autoFocus={true}  /> <br />
   <textarea value={blog.content} className="lg:text-2xl text-md mt-4 lg:mt-0 w-full focus:outline-none"  onChange={(e)=>{setBlog({...blog,content:e.target.value})}}   maxLength={5000000000} rows={4000} placeholder="Tell your story..."/>
   </div>
</div>
