    ...
    <>
      <div className="z-10 flex flex-col  items-center justify-center  absolute left-1/2 translate-x-[-50%] md:translate-y-[15%] lg:translate-y-[25%] sm:translate-y-[10%] translate-y-[5%]      rounded-lg bg-slate-200 w-3/4 h-fit  p-4 md:p-6 pb-8 lg:w-2/5">
        <div
          ...
          className="self-end  pr-2 mb-4 cursor-pointer"
        >
          <img
            width="30"
            height="30"
        ...
        <div className="w-full lg:w-2/3">
          ...
          <InputBox
            ...
            onChange={(e) => {
              setProfileSection({ ...profilesection, name: e.target.value });
            }}
          ></InputBox>
          <TextArea
            name={"Bio"}
            placeholder=""
            value={profilesection.bio || ""}
            onChange={(e) => {
              setProfileSection({ ...profilesection, bio: e.target.value });
            }}
          ></TextArea>
          <Button
            ...
            onClick={async () => {
              try {
                const res = await axios.put(
                  "https://backend.aman-lalwani208.workers.dev/api/v1/profile",
                  profilesection,
                  {
                    headers: {
        ...
        <div className=" w-full lg:w-2/3 mt-6">
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const res = await axios.put(
                  "https://backend.aman-lalwani208.workers.dev/api/v1/profile/changepassword",
                  { password },
                  {
                    headers: {
                      Authorization: localStorage.getItem("token"),
